...
import com.aetherar.launcher.LauncherProvider;
import com.aetherar.launcher.LauncherSettings;
...
import com.cyanogenmod.trebuchet.widget.MirrorSourceFrameLayout;
import com.cyanogenmod.trebuchet.widget.MirrorViewHelper;
...
public final class Launcher extends Activity
    ...
    private final int MSG_UPDATE_FPS = 2;
...
//    private IMirrorView mMirrorViewWorkspace;
//    private IMirrorView mMirrorViewApp;
    
    private MirrorViewHelper mMirrorHelper = null;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (PreferencesProvider.USE_MIRROR_VIEW) {
            int orientation = getResources().getConfiguration().orientation;
            PreferencesProvider.setMirrorEnabled(orientation == Configuration.ORIENTATION_LANDSCAPE);
        }
        
        setContentView(R.layout.launcher);
        
        if (PreferencesProvider.isMirrorEnabled()) {
            mMirrorHelper = MirrorViewHelper.getInstance();
        }
    ...
    private void checkRedirectDailog() {
    	int action = PreferencesProvider.Application.getPopupAction(0);
        if (action == 0 || action == 2) {
	        int dueCount = PreferencesProvider.Application.getPopupDueCount(0);
	        if (dueCount >= 3) {
	        	showRedirectDialog();
	        } else {
	        	PreferencesProvider.Application.setPopupDueCount(++dueCount);
	        }
        }
    }
    
	private void showRedirectDialog() {
		Dialog dlg = new AlertDialog.Builder(this).setTitle("Notification")
				.setMessage("\n       Visit our website for more information.    \n")
				.setPositiveButton("Don't Nag me", new OnClickListener(){

					@Override
					public void onClick(DialogInterface arg0, int arg1) {
						PreferencesProvider.Application.setPopupAction(1);
					}
				...
				.setNeutralButton("Later", new OnClickListener(){
					...
					@Override
					public void onClick(DialogInterface arg0, int arg1) {
						PreferencesProvider.Application.setPopupAction(2);
						PreferencesProvider.Application.setPopupDueCount(0); //clear
					}
				})
				.setNegativeButton("Visit Site", new OnClickListener(){

					@Override
					public void onClick(DialogInterface arg0, int arg1) {
						PreferencesProvider.Application.setPopupAction(3);
						Intent myIntent = new Intent(Intent.ACTION_VIEW, Uri.parse("http://aetherar.com"));
...
//        if (PreferencesProvider.isMirrorEnabled()) {
//            mMirrorViewApp = MirrorViewHelper.mirror(mAppsCustomizeContent);       
//        }
    ...
    private final Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            } else if (msg.what == MSG_UPDATE_FPS) {
            	if (mMirrorHelper != null) {
            		AppsGLSurfaceView glview = (AppsGLSurfaceView)(mMirrorHelper.getMirrorView());
            		if (glview != null && mTVFPS != null) {
		            	mTVFPS.setText(" FPS : " + glview.getFPS());
		            	mHandler.sendEmptyMessageDelayed(MSG_UPDATE_FPS, 1000);
            		}
            	}
            }
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        // if device screen lock set to none and we do lock/unlock screen 
        // onCreate will not get called because we are still the top in the stack
        // so in this case mOnResumeState is actually mState
        mOnResumeState = mState;
...
//            appSearchData, sourceBounds);
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        ...
        if (item != null) setVisible(preferencesVisible);
        return true;
    }
    ...
    void setWallpaperVisibility(boolean visible) {
        if (PreferencesProvider.isMirrorEnabled()) {
            // disable wallpaper since we use camera preview
            visible = false;
        }
    ...
    private void setWorkspaceBackground(boolean workspace) {
        if (mLauncherView != null) {
            if (PreferencesProvider.isMirrorEnabled()) {
                mLauncherView.setBackgroundColor(0);    
            } else {
