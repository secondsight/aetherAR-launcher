...
public class PagedViewCellLayoutChildren extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        double cellWidth = r - l;
        double cellHeight = b - t;
        Log.d(TAG, "container w = " + cellWidth + " h = " + cellHeight);
        for (int i = 0; i < count; i++) {
            View child = getChildAt(i);
            if (child.getVisibility() != GONE) {
                PagedViewCellLayout.LayoutParams lp =
                ...
                if (PreferencesProvider.isMirrorEnabled() && !PreferencesProvider.USE_GL_MIRROR) {
                    double glX = -0.5 + (childLeft+lp.width/2) / cellWidth;
                    double glY = -(childTop+lp.height/2) / cellHeight + 0.5;
                    float rx = (float)(Math.sin(glY) * 180 / Math.PI);
                    child.setRotationX(-rx);
                    float ry = (float)(Math.sin(glX) * 180 / Math.PI);
                    child.setRotationY(-ry);
                    
                    double rawDis = glX * glX + glY * glY;
                    double dis = rawDis * rawDis; // we want only slightly scale the icons
                    float scale = (float)dis + 0.9f;
                    child.setScaleX(scale);
                    child.setScaleY(scale);
                    
                    child.setAlpha((float)(1 - rawDis));                    
                }
            }
        }
    }
